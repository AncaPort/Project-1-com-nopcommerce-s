...
public class TestSuite extends Utility {
    ...
    public void verifyProductArrangeInAlphaBaticalOrder() throws InterruptedException {
        ...
        //1.4 Verify the Product will arrange in Descending order.
        List<WebElement> beforeFilterNames = driver.findElements(By.cssSelector(".product-title"));
        List<String> beforeFilterList = new ArrayList<>();
        for (WebElement p : beforeFilterNames) {
            beforeFilterList.add(p.getText());
        }
        Collections.sort(beforeFilterList);
        Collections.reverse(beforeFilterList);
        selectByValueFromDropDown(By.id("products-orderby"), "6");
        Thread.sleep(2000);
        List<WebElement> afterFilterNames = getMultipleElements(By.className("product-title"));
        List<String> afterFilterList = new ArrayList<>();
        for (WebElement s : afterFilterNames) {
            afterFilterList.add(s.getText());
        }
        Assert.assertEquals("Products are not sorted in descending order", afterFilterList, beforeFilterList);
    }
    ...
    public void verifyProductAddedToShoppingCartSuccessFully() throws InterruptedException {
        ...
        //2.14 Then MouseHover on "Shopping cart" and Click on "GO TO CART" button.
        mouseHoverAndClickToElement(By.xpath("//span[text()='Shopping cart']"));
        Thread.sleep(3000);
...
//            2.20 Verify the Text “Welcome, Please Sign In!”
        ...
        //2.22 Fill the all mandatory field
        sendTextToElement(By.id("BillingNewAddress_FirstName"), "Shital");   //insert username
        sendTextToElement(By.id("BillingNewAddress_LastName"), "Sanghani");         //insert lastname
        sendTextToElement(By.id("BillingNewAddress_Email"), "shital123@gmail.com");    //insert random email id
        ...
        sendTextToElement(By.id("BillingNewAddress_Address1"), "123 Whitchurch Avenue");   //insert address
        sendTextToElement(By.id("BillingNewAddress_ZipPostalCode"), "NW97SF");           //insert postcode
        sendTextToElement(By.id("BillingNewAddress_PhoneNumber"), "07654321890");     //enter user phone number
        Thread.sleep(2000);
        //2.23 Click on “CONTINUE”
        ...
        //2.28 Fill all the details
        sendTextToElement(By.id("CardholderName"), "Shital");
